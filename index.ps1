...
)
{
    ...
    {
        $line = $lines[$i]
        $matched = [System.Text.RegularExpressions.Regex]::Matches(
            $line, 
            "(?:\s*using\s)([\w\.]+)(?:\s*;\s*)")
    ...
    try
    {
        ...
        using System;
            public class Program {
                    ...
                    {
                        AutoClickerApp.App app = new AutoClickerApp.App();
                        app.Start();
                    }
                }
"@ `
...
# bundle together all .cs files
$sourceCode = ConcatinateCodeFiles `
    -codeFilesPath $SOURCE_CODE_PATH

# write bundled code in file (for debugging purposes)
Set-Content `
    -Path $BUNDLED_CODE_PATH `
    -Value $sourceCode

# compile bundled code to .net assembly
CompileToAssembly `
    -outputAssemblyName $ASSEMBLY_PATH `
    -sourceCode $sourceCode

# wrap the generated assembly in new program (either in-memory or as .exe)
GenerateExecutable `
